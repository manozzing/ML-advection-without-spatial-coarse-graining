...
### Single run for domain/parameters loading once
...
for Temp_factor in [4, 8, 12, 16, 32]
    ...
    for Vertical_level in 1.0:1:72.0
        ModelingToolkit.check_units(eqs...) = nothing; start = Dates.datetime2unix(Dates.DateTime(2018, Starting_month, 1))
        @parameters t
        lons = collect(-130.0:0.3125*space_factor:-60.0); lats = collect(9.75:0.25*space_factor:60.0); levs = collect(Vertical_level:1:Vertical_level)
        nsims = length(lats)*length(lons)*length(levs)
...
r2 = readdlm("gen_level_r2.txt")
plot(r2[:,2], r2[:,1],
    xlabel="R²", ylabel="Vertical level", xlabelfontsize=14, ylabelfontsize=14, 
    xtickfontsize=12, ytickfontsize=12, linewidth=2,
    xlim=(0,1), ylim=(1,72), label="4dt", legendfontsize=12,
    top_margin = 0mm, bottom_margin = 8mm, left_margin = 10mm, right_margin = 2mm)
plot!(r2[:,3], r2[:,1], linewidth=2, label="8dt", legendfontsize=12)
plot!(r2[:,4], r2[:,1], linewidth=2, label="12dt", legendfontsize=12)
plot!(r2[:,5], r2[:,1], linewidth=2, label="16dt", legendfontsize=12)
plot!(r2[:,6], r2[:,1], linewidth=2, label="32dt", legendfontsize=12)
plot!(size=(300,400))
savefig("r2_plot_level.pdf")


max_cfl_levels = readdlm("max_cfl_levels.txt") 

plot(max_cfl_levels, 1:1:72, xlabel="Maximum CFL number", ylabel="Vertical layer", linewidth=3,
xtickfontsize=12, ytickfontsize=12, xlabelfontsize=14, ylabelfontsize=14, legend=false,
top_margin = 0mm, bottom_margin = 10mm, left_margin = 12mm, right_margin = 5mm,
xlim=(0, 2.5))
plot!(size=(300,400))
savefig("max_cfl_levels.pdf")
